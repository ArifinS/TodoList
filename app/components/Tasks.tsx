...
import { Trash2, FilePenLine, Eye, Sun, Moon } from "lucide-react";
...
const Tasks = () => {
  ...
  if (!tasks) {
    return (
      <div className={`text-center py-8 text-lg ${!isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
        No tasks to display
      </div>
    );
  }
  return (
    <section className="mb-20" id="tasks">
      <div className="container mx-auto">
        <div className={`rounded-xl border ${!isDarkMode ? 'border-[rgba(206,206,206,0.12)] bg-[#1D212B]' : 'border-gray-200 bg-white'} px-6 py-8 md:px-9 md:py-16`}>
          <div className="mb-14 items-center justify-between sm:flex">
            <h2 className={`text-2xl font-semibold ${!isDarkMode ? 'text-white' : 'text-gray-900'} max-sm:mb-4`}>Your Tasks</h2>
            <div className="flex items-center space-x-5">
              ...
              <CustomAlertDialog
                trigger={
                  <button
                    ...
                    className={`rounded-lg bg-gradient-to-r from-red-500 to-red-600 px-3.5 py-2.5 text-sm font-semibold text-white hover:from-red-600 hover:to-red-700 transition-all duration-300 ${tasks.length === 0 ? "opacity-50 cursor-not-allowed" : "cursor-pointer"}`}
                  >
                    Delete All Tasks
                  </button>
                }
              ...
              <button
                onClick={toggleTheme}
                className={`rounded-lg p-2.5 text-lg font-semibold ${!isDarkMode ? 'bg-gray-700 text-white hover:bg-gray-600' : 'bg-gray-200 text-gray-900 hover:bg-gray-300'} transition-all duration-300 cursor-pointer`}
              ...
              Object.entries(groupedTasks).map(([group, groupTasks]) =>
                groupTasks.length > 0 ? (
                  <div key={group} className="mb-8">
                    ...
                    <table className={`table-fixed w-full ${!isDarkMode ? 'text-white' : 'text-gray-900'}`}>
                      <thead>
                        <tr className={`border-b ${!isDarkMode ? 'border-[#2E3443]' : 'border-gray-200'}`}>
                          {groupBy !== "Favorites" && (
                            <th className="p-4">★</th>
                          )}
                      ...
                      <tbody>
                        {groupTasks.map((task) => (
                          <tr key={task.id} className={`border-b ${!isDarkMode ? 'border-[#2E3443]' : 'border-gray-200'}`}>
                            {groupBy !== "Favorites" && (
                              <td className="text-center">
                                <svg
                                  ...
                                  viewBox="0 0 24 24"
                                  strokeWidth="2"
                                  stroke={task.starred ? "yellow" : !isDarkMode ? "white" : "currentColor"}
                            ...
                            <td className="p-4 truncate max-w-[100px]">
                              <Tooltip>
                                ...
                                <TooltipContent className={`${!isDarkMode ? 'bg-[#2D333F] text-white border-gray-600' : 'bg-white text-gray-900 border-gray-200'}`}>
                                  <p>{task.title}</p>
                                </TooltipContent>
                              </Tooltip>
                            </td>
                              ...
                              <td>
                                <ul className="flex flex-wrap gap-1.5">
                                  {task.tags.map((tag, i) => (
                                    <li key={i}>
                                      <span
                                        className={`inline-block rounded px-2 py-0.5 text-sm ${!isDarkMode ? 'text-white' : 'text-gray-900'} ${task.tagColors[i]}`}
                                      >
                                        {tag}
                                      </span>
                                    </li>
                            ...
                            <td className="text-center">
                              <div className="flex justify-center gap-3">
                                <button
                                  className={`transition-colors duration-200 cursor-pointer ${!isDarkMode ? 'text-blue-400 hover:text-blue-300' : 'text-blue-500 hover:text-blue-600'}`}
                                ...
                                <CustomAlertDialog
                                  trigger={
                                    <button className={`transition-colors duration-200 cursor-pointer ${!isDarkMode ? 'text-red-400 hover:text-red-300' : 'text-red-500 hover:text-red-600'}`}>
                                      <Trash2 />
                                    </button>
                                  }
                                  title="Are you absolutely sure?"
